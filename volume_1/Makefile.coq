...
SEvolume_1 := $(lastword $(MAKEFILE_LIST))
...
Mvolume_1ILES           := $(COQMF_Mvolume_1ILES)
...
DOCDIR            := $(COQMF_DOCDIR)
OCAMvolume_1IND         := $(COQMF_OCAMvolume_1IND)
CAMvolume_1LAGS         := $(COQMF_CAMvolume_1LAGS)
HASNATDYNLINK     := $(COQMF_HASNATDYNLINK)
OCAMLWARN         := $(COQMF_WARN)

Makefile.coq.conf: 
	coq_makefile -Q . volume_1 -o Makefile.coq Preface.v Basics.v Induction.v Lists.v Poly.v Tactics.v Logic.v IndProp.v Maps.v ProofObjects.v IndPrinciples.v Rel.v Imp.v ImpParser.v ImpCEvavolume_1un.v Extraction.v Auto.v AltAuto.v Postscript.v Bib.v PrefaceTest.v BasicsTest.v InductionTest.v ListsTest.v PolyTest.v TacticsTest.v LogicTest.v IndPropTest.v MapsTest.v ProofObjectsTest.v IndPrinciplesTest.v RelTest.v ImpTest.v ImpParserTest.v ImpCEvavolume_1unTest.v ExtractionTest.v AutoTest.v AltAutoTest.v PostscriptTest.v BibTest.v
...
# OCaml binaries
CAMLC       ?= "$(OCAMvolume_1IND)" ocamlc   -c
CAMLOPTC    ?= "$(OCAMvolume_1IND)" opt      -c
CAMLLINK    ?= "$(OCAMvolume_1IND)" ocamlc   -linkall
CAMLOPTLINK ?= "$(OCAMvolume_1IND)" opt      -linkall
CAMLDOC     ?= "$(OCAMvolume_1IND)" ocamldoc
CAMLDEP     ?= "$(OCAMvolume_1IND)" ocamldep -slash -ml-synonym .mlpack
...
findlib_install = \
	$(HIDE)if [ "$(METAFILE)" ]; then \
	  ...
	  "$(OCAMvolume_1IND)" install $(2) $(FINDLIBDESTDIR) $(FINDLIBPACKAGE) $(1); \
	  rc=$$?; \
...
findlib_remove = \
	$(HIDE)if [ ! -z "$(METAFILE)" ]; then\
	  "$(OCAMvolume_1IND)" remove $(FINDLIBDESTDIR) $(FINDLIBPACKAGE); \
	fi
...
# Coq's own core libraries, which should be replaced by ocamvolume_1ind
# options at some point.
COQ_SRC_SUBDIRS?=
COQSRCLIBS?= $(foreach d,$(COQ_SRC_SUBDIRS), -I "$(COQLIB)/$(d)")

CAMvolume_1LAGS+=$(OCAMLLIBS) $(COQSRCLIBS)
# ocamldoc fails with unknown argument otherwise
CAMLDOCFLAGS:=$(filter-out -annot, $(filter-out -bin-annot, $(CAMvolume_1LAGS)))
CAMvolume_1LAGS+=$(OCAMLWARN)
...
CMXSFILES = \
	...
	$(if $(MLPACKFILES)$(CMXAFILES),,\
		$(MLGFILES:.mlg=.cmxs) $(Mvolume_1ILES:.ml=.cmxs))
...
all:
	$(HIDE)$(MAKE) --no-print-directory -f "$(SEvolume_1)" pre-all
	$(HIDE)$(MAKE) --no-print-directory -f "$(SEvolume_1)" real-all
	$(HIDE)$(MAKE) --no-print-directory -f "$(SEvolume_1)" post-all
.PHONY: all

all.timing.diff:
	$(HIDE)$(MAKE) --no-print-directory -f "$(SEvolume_1)" pre-all
	$(HIDE)$(MAKE) --no-print-directory -f "$(SEvolume_1)" real-all.timing.diff TIME_OF_PRETTY_BUILD_EXTRA_FILES=""
	$(HIDE)$(MAKE) --no-print-directory -f "$(SEvolume_1)" post-all
...
pretty-timed:
	...
	$(HIDE)$(MAKE) --no-print-directory -f "$(SEvolume_1)" print-pretty-timed
...
# FIXME, see Ravolume_1's bugreport
...
install: META
	...
	$(HIDE)$(MAKE) install-extra -f "$(SEvolume_1)"
	@rm -f .filestoinstall
install-extra::
	@# Extension point
...
$(MLIFILES:.mli=.cmi): %.cmi: %.mli
	$(SHOW)'CAMLC -c $<'
	$(HIDE)$(TIMER) $(CAMLC) $(CAMLDEBUG) $(CAMvolume_1LAGS) $(FINDLIBPKGS) $<
...
$(filter %.cmo, $(Mvolume_1ILES:.ml=.cmo) $(MLGFILES:.mlg=.cmo)): %.cmo: %.ml
	$(SHOW)'CAMLC -c $<'
	$(HIDE)$(TIMER) $(CAMLC) $(CAMLDEBUG) $(CAMvolume_1LAGS) $(FINDLIBPKGS) $<

# Same hack
$(filter %.cmx, $(Mvolume_1ILES:.ml=.cmx) $(MLGFILES:.mlg=.cmx)): %.cmx: %.ml
	$(SHOW)'CAMLOPT -c $(FOR_PACK) $<'
	$(HIDE)$(TIMER) $(CAMLOPTC) $(CAMLDEBUG) $(CAMvolume_1LAGS) $(FINDLIBPKGS) $(FOR_PACK) $<


$(MLLIBFILES:.mllib=.cmxs): %.cmxs: %.cmxa
	$(SHOW)'CAMLOPT -shared -o $@'
	$(HIDE)$(TIMER) $(CAMLOPTLINK) $(CAMLDEBUG) $(CAMvolume_1LAGS) $(FINDLIBPKGS) \
		-shared -o $@ $<

$(MLLIBFILES:.mllib=.cma): %.cma: | %.mllib
	$(SHOW)'CAMLC -a -o $@'
	$(HIDE)$(TIMER) $(CAMLLINK) $(CAMLDEBUG) $(CAMvolume_1LAGS) $(FINDLIBPKGS) -a -o $@ $^

$(MLLIBFILES:.mllib=.cmxa): %.cmxa: | %.mllib
	$(SHOW)'CAMLOPT -a -o $@'
	$(HIDE)$(TIMER) $(CAMLOPTLINK) $(CAMLDEBUG) $(CAMvolume_1LAGS) $(FINDLIBPKGS) -a -o $@ $^
...
$(MLPACKFILES:.mlpack=.cmo): %.cmo: | %.mlpack
	$(SHOW)'CAMLC -pack -o $@'
	$(HIDE)$(TIMER) $(CAMLLINK) $(CAMLDEBUG) $(CAMvolume_1LAGS) $(FINDLIBPKGS) -pack -o $@ $^

$(MLPACKFILES:.mlpack=.cmx): %.cmx: | %.mlpack
	$(SHOW)'CAMLOPT -pack -o $@'
	$(HIDE)$(TIMER) $(CAMLOPTLINK) $(CAMLDEBUG) $(CAMvolume_1LAGS) $(FINDLIBPKGS) -pack -o $@ $^

# This rule is for _CoqProject with no .mllib nor .mlpack
$(filter-out $(MLLIBFILES:.mllib=.cmxs) $(MLPACKFILES:.mlpack=.cmxs) $(addsuffix .cmxs,$(PACKEDFILES)) $(addsuffix .cmxs,$(LIBEDFILES)),$(Mvolume_1ILES:.ml=.cmxs) $(MLGFILES:.mlg=.cmxs)): %.cmxs: %.cmx
...
$(addsuffix .timing.diff,$(VFILES)): %.timing.diff : %.before-timing %.after-timing
	...
	$(HIDE)$(MAKE) --no-print-directory -f "$(SEvolume_1)" print-pretty-single-time-diff BEFORE=$*.before-timing AFTER=$*.after-timing TIME_OF_PRETTY_BUILD_FILE="$@"
...
$(HTMvolume_1ILES): %.html: %.v %.glob
	$(SHOW)'COQDOC -html $<'
	$(HIDE)$(COQDOC) $(COQDOCFLAGS) -html $< -o $@

$(GHTMvolume_1ILES): %.g.html: %.v %.glob
	$(SHOW)'COQDOC -html -g $<'
...
GENMvolume_1ILES:=$(MLGFILES:.mlg=.ml)
$(addsuffix .d,$(ALLSRCFILES)): $(GENMvolume_1ILES)
...
$(addsuffix .d,$(Mvolume_1ILES)): %.ml.d: %.ml
	$(SHOW)'CAMLDEP $<'
	$(HIDE)$(CAMLDEP) $(OCAMLLIBS) "$<" $(redir_if_ok)
...
byte:
	$(HIDE)$(MAKE) all "OPT:=-byte" -f "$(SEvolume_1)"
.PHONY: byte

opt:
	$(HIDE)$(MAKE) all "OPT:=-opt" -f "$(SEvolume_1)"
.PHONY:	opt
...
printenv::
	...
	@echo 'DOCDIR = $(DOCDIR)'
	@echo 'OCAMvolume_1IND = $(OCAMvolume_1IND)'
...
.merlin:
	$(SHOW)'FILL .merlin'
	$(HIDE)echo 'FLG $(COQMF_CAMvolume_1LAGS)' > .merlin
	...
	$(HIDE)$(MAKE) merlin-hook -f "$(SEvolume_1)"
.PHONY: merlin
